...
use crate::{
    ...
    pin::*,
    systems::utils::compute_chip_height,
};
...
impl Renderer<'_> {
    ...
    pub fn draw_connection(&mut self, from: Vector2, to: Vector2, thick: f32, color: Color) {
    ...
    pub fn draw_pin(&mut self, org: Vector2, pin: &Pin) {
        ...
        match kind {
            PinKind::Constant => {
                self.handle
                    .draw_circle_lines(org.x as i32, org.y as i32, PIN_RADIUS + 3.0, color)
            }
            _ => {}
        }
    }
    ...
    pub fn draw_chip(&mut self, pos: Vector2, chip: &Chip) {
        ...
        self.handle
            .draw_rectangle_v(pos, Vector2::new(CHIP_WIDTH, height), Color::ORANGE);
    }

    pub fn draw_text(&mut self, pos: Vector2, text: &str) {
        self.handle
            .draw_text(text, pos.x as i32, pos.y as i32, 20, Color::BLACK);
    }
}
